...
update_da(){
  rsync -Aar --delete $rsync_workdir/$service/da/ deploy
  if [ $? -ne 0 ];then
    logex 2 "[FATAL]rsync failed. svr:$rsync_workdir/$service/da/"
    return 1
  fi

  dstr=$(date +%Y%m%d_%H%M%S)

  cp -rf deploy/bin/{conf,shell,tools} bin
  cp -rf deploy/conf ./
...
update_bc(){
  rsync -Aar --delete $rsync_workdir/$service/bc/ deploy
...
update_bs(){
  rsync -Aar --delete $rsync_workdir/$service/bs/ deploy
  if [ $? -ne 0 ];then
    logex 2 "[FATAL]rsync failed. svr:$rsync_workdir/$service/bs/"
    return 1
  fi

  csort=csort_$service.so
  ...
  cp -rf deploy/bin/{conf,shell} bin
  mkdir -p bin/tools
  cp -rf deploy/bin/tools/*.{ini,sh,py}  bin/tools >&/dev/null
